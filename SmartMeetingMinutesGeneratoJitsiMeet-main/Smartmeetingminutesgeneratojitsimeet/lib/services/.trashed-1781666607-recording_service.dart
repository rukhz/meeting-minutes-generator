...
class RecordingService {
  ...
  Future<String?> autoDetectServerUrl({
    ...
    final priorityIps = <int>[1, 2, 5, 10, 50, 100, 101];
    try {
    ...
    candidateList.sort((a, b) {
      try {
        final hostA = Uri.parse(a).host;
        final hostB = Uri.parse(b).host;
        final lastA = int.tryParse(hostA.split('.').last) ?? 999;
        final lastB = int.tryParse(hostB.split('.').last) ?? 999;
        final idxA = priorityIps.indexOf(lastA);
        final idxB = priorityIps.indexOf(lastB);
  ...
  /// [jitsiServerUrl] - Jitsi base URL (e.g. https://meet.jit.si or https://8x8.vc).
  /// Use same server as your meeting so the bot joins the correct room.
  Future<String?> startRecording({
    ...
    if (jitsiServerUrl != null && jitsiServerUrl.trim().isNotEmpty) body['jitsiServerUrl'] = jitsiServerUrl.trim();
  ...
  Future<List<File>> getRecordings() async {
    try {
      ...
      final allowedExt = ['.mp4', '.webm', '.m4a', '.mp3'];
      final files = recordingsDir
          .listSync()
          .whereType<File>()
          .where((file) {
            final pathLower = file.path.toLowerCase();
            return allowedExt.any((ext) => pathLower.endsWith(ext));
          })
          .toList();
